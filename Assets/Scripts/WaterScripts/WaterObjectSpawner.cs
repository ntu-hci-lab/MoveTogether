...
public class WaterObjectSpawner : MonoBehaviour
{
    ...
    public NetworkObject trampoline;
    ...
    public float originalControllerDistance;
    private float lastControllerDistance;
    private bool isSqueezed = false;

    public float squeezeThreshold = 0.2f; // Threshold for detecting squeeze
    ...
    private void OnLoaded(NetworkRunner networkRunner)
    {
        ...
        originalControllerDistance = Vector3.Distance(leftHandTransform.position, rightHandTransform.position);
        lastControllerDistance = originalControllerDistance;
    ...
    void Update()
    {   
        float currentControllerDistance = Vector3.Distance(leftHandTransform.position, rightHandTransform.position);
        ...
        if (level == 2) {
            ...
            {   
                // Switch mode with primary trigger
                ...
                if (OVRInput.GetDown(OVRInput.Button.SecondaryIndexTrigger))
                {
                    ...
                    else if(IsBubblegunGrabbed() && mode == 0)
                    {
                        SpawnBubble();
                    }
                ...
                if (mode == 1)
                {
                    BlowBubble(); // Continuously blow bubbles when mode is 1
                }

                if (mode == 0)
                {
                    // Check for squeeze
                    if (!isSqueezed && currentControllerDistance < lastControllerDistance - squeezeThreshold) // Detect squeeze
                    {
                        isSqueezed = true;
                    ...
                    // Check for release
                    if (isSqueezed && currentControllerDistance > lastControllerDistance + squeezeThreshold) // Detect release
                    {
                        SpawnBubble(); // Perform the bubble spawning action
                        isSqueezed = false; // Reset squeeze state
                        lastControllerDistance = currentControllerDistance; // Update last distance after release
                    }
                }
            ...
            // }
                ...
                {   
                    // print current distance last distance and isSqueezed
                    distanceText.text = $"CD: {currentControllerDistance}, LD: {lastControllerDistance}, Squeezed: {isSqueezed}";

                    // print rotation
                    // distanceText.text = $"CR: {cumulativeRotationX}, Current: {currentRotationX}, Last: {lastControllerRotationX}";
                }
        }
    ...
    private void BlowBubble()
    {
        ...
        {
            float distance = Vector3.Distance(Bubblegun.transform.position, bubble.transform.position);
            if (distance <= blowRange)
            {
                ...
                Vector3 blowDirection = (bubble.transform.position - Bubblegun.transform.position).normalized;
    ...
    private void SpawnBubble()
    {
        if (_networkRunner != null)
        {
            Vector3 spawnPosition = Bubblegun.transform.position + Bubblegun.transform.forward * 0.7f; // Adjust spawn distance
            _networkRunner.Spawn(
                bubble,
                spawnPosition,
                Bubblegun.transform.rotation,
                ...
                (runner, obj) =>
                {
                    obj.transform.localScale = new Vector3(bubbleSize, bubbleSize, bubbleSize); // Adjust size
                }
            );
        }
    }
